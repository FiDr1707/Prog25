...
class MyClass {
  ...
  public int b; //открытый доступ, значение переменной может изменяться из любого класса
  int c; //доступ по умолчанию, в данном случае - аналог открытого доступа
  //Методы доступа к закрытой переменной а
  void setA (intA) {
    if (A < 10)
      a= A;
    else {
...
class FailSoftArray {
  ...
  //Объявление закрытой ссылки на массиви и закрытого значения ошибки доступа
  private int a [];
  private int errval;
  ...
  //Метод доступа к элементам массива при изменеии значения элемента по индексу
...
//Демострация передачи объектов методам
class Block {
  int a, b, c;
  int V;
...
class ErrorInfo {
  ...
  Err getErrorInfo(int i) {
    if (i >= 0 & i < msgs.length)
      ...
      return "Неверное значение ошибки", 0;
  }
}
class Pr012 {
  public static void main (String args[]) {
    ...
    ob.b = 10;
    ob.c = 10;
    System.out.println("Значение переменной а в объекте ob: " + ob.b);
    System.out.println("Значение переменной а в объекте ob: " + ob.c);
  ...
  System.out.println("\nob2 совпадает по размерам с ob3: " + ob2.sameV(ob3));
